...
//                    .type("Microservice")).getId();
    ...
    private static void createPackAndPublishPipeline(@Nullable Direction direction, ConnectableFlowable<ConnectivityMessage> messageConnectable,
        ...
        batchConnectable.map(ConnectivityBatch::convertToProtoRawBatch).doOnNext(it -> rawMessageRouter.send(it, direction == Direction.FIRST ? "in" : "out", "raw"));
        LOGGER.info("Subscribed to transfer raw batch group {}", direction);

        batchConnectable.map(ConnectivityBatch::convertToProtoParsedBatch).doOnNext(it -> parsedMessageRouter.send(it, direction == Direction.FIRST ? "in" : "out", "parsed"));
